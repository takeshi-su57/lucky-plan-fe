...
export function WorkerDetails({
  ...
  return (
    <main
      className={`mx-auto w-full max-w-6xl space-y-5 ${embedded ? "pb-6" : "px-4 py-6 sm:px-6"}`}
    >
            ...
            {worker.platformCaches.map((cache) => (
              ...
              >
                ...
                <p className="text-default-500 mt-2 text-xs">
                  {stamp(cache.coveredStartAt)} â€” {stamp(cache.coveredEndAt)}
                </p>
                {cache.lastError && (
    ...
    <div className="border-default-100 grid gap-3 border-b px-5 py-4 md:grid-cols-[minmax(0,1.2fr)_minmax(180px,.8fr)_auto]">
      <div className="min-w-0">
        ...
        <p className="text-default-500 mt-1 text-xs">
          Created {stamp(task.createdAt)} Â· Range {stamp(task.rangeStartedAt)}{" "}
          â€” {stamp(task.rangeEndedAt)}
        </p>
        {task.claimedAt && (
          <p className="text-default-500 mt-1 text-xs">
            {task.completedAt
              ? `Completed ${stamp(task.completedAt)}${duration(task.claimedAt, task.completedAt) ? ` Â· Took ${duration(task.claimedAt, task.completedAt)}` : ""}`
          ...
          <div className="bg-default-50 mt-3 rounded-lg p-2 text-xs">
            <div className="flex flex-wrap justify-between gap-x-3 gap-y-1">
              <span className="font-medium">
                Evaluation timing Â· {timing.totalMs}ms
              </span>
      ...
      <div>
        ...
        {task.status === "Claimed" && (
          <>
            ...
            />
            <p className="text-default-500 mt-1 text-xs">
              {task.progressPercent.toFixed(0)}% Â·{" "}
