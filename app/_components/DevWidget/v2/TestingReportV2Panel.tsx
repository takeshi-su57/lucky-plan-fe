...
import { TestingReportV2 } from "@/graphql/gql/graphql";

import { useGetTestingReportV2 } from "@/app/_hooks/useHistory";

import { Virtuoso } from "react-virtuoso";
import { getPriceStr } from "@/utils/price";
import LineChart from "@/components/charts/LineChart";

export function TestingReportV2Panel() {
  const { reports, loading, fetchMore, hasMore } = useGetTestingReportV2();
...
export function ReportView({ report }: { report: TestingReportV2 }) {
  let usdPnl = 0;
  ...
  return (
    <Card>
      <CardBody className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          <span>
            Slope: {report.minSlope} - {report.maxSlope}
          </span>
        </div>

        <div className="flex items-center gap-3">
          <span>Close Counts: </span>

          {Object.entries(JSON.parse(report.r2MinsByPnlSnapshotKind)).map(
            ([key, value]) => (
              <span key={key}>
                {key}: {value as any}
              </span>
