...
import { useGetWholeCompressedHistoriesV3 } from "@/app/_hooks/useHistory";

import BarChart from "@/components/charts/BarChart";
import LineChart from "@/components/charts/LineChart";
import { getPriceStr } from "@/utils/price";
import { TestParamsV3 } from "./TestParamsV3";
...
export type FastTotalDevV3PanelProps = {
  testParams: TestParamsV3[];
};

export function FastTotalDevV3Panel({ testParams }: FastTotalDevV3PanelProps) {
  const {
    accPnls,
    botCounts,
    actionTypeCount,
    ...
    maxInvested,
    loading,
  } = useGetWholeCompressedHistoriesV3(testParams);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div>
        {!!actionTypeCount
          ? Object.entries(JSON.parse(actionTypeCount as string)).map(
              ([key, value]) => (
                <div key={key}>
                  {key}: {value as any}
                </div>
      ...
      <span>Unique Traders: {uniqueTraders.length}</span>

      <div className="flex flex-col gap-2">
        <TradeHistories walletAddresses={uniqueTraders} />
      </div>
    </div>
  );
}
