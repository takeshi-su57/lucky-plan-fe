...
import { useGetTestingReport } from "@/app/_hooks/useHistory";
  ...
  return (
    <Card>
      <CardBody className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          <span>Recent Traded Days: {report.recentTradedDays}</span>
          <span>
            Slope: {report.minSlope} - {report.maxSlope}
          </span>
          <span>
            R2: {report.minR2} - {report.maxR2}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {Object.entries(
            JSON.parse(report.closePositionCountsByPnlSnapshotKind),
          ).map(([key, value]) => (
            <span key={key}>
              {key}: {value as any}
            </span>
